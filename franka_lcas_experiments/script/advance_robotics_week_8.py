            ...
            if self.img_msg_received and self.depth_msg_received:
                ...
                if k==27:    # Esc key to stop
                    ...
                    if k == ord('s'): 
                        ...
                        cv2.imwrite( os.path.join( self.BASE_PATH , 'image.png' ) , self.img_msg)
                        saved_img = np.reshape( self.img_msg, (1,256,256,3))
                        np.save( os.path.join( self.BASE_PATH, 'image_xy.npy' ) , saved_img )
                        target_point = np.array( [self.imgX, self.imgY ] ) 
                        np.save( os.path.join( self.BASE_PATH, 'image_target_xy.npy' ), target_point )
                    ...
                    # if k == ord('w'):
                    #     command = ["python3", "/home/arshad/franka_ws/src/franka_ros_lcas/franka_lcas_experiments/script/load_model.py"]
                    #     subprocess.call(command)
                    ...
                    if k == ord('p'):
                        traj_true = np.load( os.path.join( self.BASE_PATH, 'predicted_joints_values_rtp.npy') )
